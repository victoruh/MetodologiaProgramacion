...
void Imagen::crear(int filas,int columnas){
        ...
        for (int i=0; i<filas; i++){
                datos[i]= new byte [columnas]; //creamos un array para el puntero del array datos[i]
              }
}
...
void Imagen::setPos(int i, byte v){
  if (i < this->nfilas*this->ncolumnas){ //Si no se sale del array entramos
    int contadorFilas = 0;
    int contadorColumnas = 0;
  for (int x=0;x<nfilas && contadorFilas+contadorColumnas <i;x++){
    contadorFilas++;
    for (int y=0;y<ncolumnas && contadorFilas+contadorColumnas<i;y++)
      contadorColumnas++;
    }
  datos[contadorFilas][contadorColumnas]=v;
    }
}
...
byte Imagen::getPos(int i){
  ...
  for (int x=0;x<nfilas && contadorFilas+contadorColumnas <i;x++){
    ...
    return datos[contadorFilas][contadorColumnas];
    }
...
const bool Imagen::escribirImagen(const char nombreFichero[], bool esBinario){
        return (esBinario) ?
               escribirPGMBinario (nombreFichero, datos[0], this->nfilas, this->ncolumnas) :
               escribirPGMTexto(nombreFichero, datos[0], this->nfilas, this->ncolumnas);
